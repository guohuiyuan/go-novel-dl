...
- 已完成章节级下载进度显示
- 已完成部分站点的真实下载验证
- 已补充一批中文站点适配器，并区分“已实测可用”与“已接入但受站点防护限制”
- Web 界面暂未开始

说明：当前最成熟的实现仍是 `esjzone`，但 `fanqienovel`、`faloo` 等中文站点已经进入可实际使用阶段。
...
| Novalpie | `novalpie` | 部分可用 | 否 | 部分 | 支持 | 支持 | 已接入登录、目录和解密链路，仍需继续校准签名逻辑稳定性 |
| 若初文学网 | `ruochu` | 部分可用 | 否 | 否 | 部分 | 否 | 已接入目录页和 JSONP 章节接口，但还缺少公开站点实测验证 |
| 番茄小说网 | `fanqienovel` | 可用 | 是 | 否 | 否 | 否 | 已实测真实下载，已接入 `__INITIAL_STATE__` 解析与字体映射 |
| 飞卢小说网 | `faloo` | 可用 | 否 | 部分 | 否 | 否 | 已实测公开章节下载；当前默认按无 Cookie 公开章节范围运行 |
| 17K 小说网 | `n17k` | 部分可用 | 是 | 否 | 部分 | 否 | 已接入目录与章节抓取及基础反爬 Cookie，但当前公开站点实测仍会被错误页拦截 |
| 红袖招 | `hongxiuzhao` | 部分可用 | 否 | 否 | 部分 | 否 | 已接入适配器和字体映射，但当前公开访问会被 Cloudflare 挡板拦截 |
