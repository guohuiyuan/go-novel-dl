...
- 下载速度估算重写：用 30 秒滑动窗口斜率替换原 EMA + 非对称 clamp + 平均速率混合算法。`MarkRunning` 重置样本缓冲，`MarkProgress` 在窗口内做几何斜率估算，rate=0 时显式清空 ETA；可正确反映稳态/加减速/卡住/单章慢/重试回退等场景，新增 11 个单元测试覆盖
- 测速接口 `runSpeedProbe` 抽象为可测函数：每站点最多采两次取最优、并发上限 8、本地超时与父 ctx 取消区分判定，结果新增 `samples` 字段，前端徽章 tooltip 注明是否取了 best-of-2
- 文件名模板可在设置中心自定义：暴露 `OutputConfig.FilenameTemplate` 到 `GeneralConfigRecord` 与 web 表单，支持 `{title}` `{author}` `{site}` `{book_id}` 四个占位符（exporter 同步扩展），sanitize 后再追加扩展名
- 下载任务卡片重构：标题/状态/操作单行展示，meta 信息改 chip 行，进度条与计数同行；下载结束的文件 chip 拆成「主文件名 + 扩展名」双 span，移动端长文件名截断也始终保留 `.txt`/`.epub` 等后缀
- 任务卡片新增「📤 重新导出」按钮：非进行中的任务都可点，弹出 popover 选 txt/epub/html 中任意组合重新生成文件，复用已下载章节，新建独立任务
- 书架去掉手动刷新按钮，每次切到书架 tab 自动重载；「新建文件夹」在子目录里完全隐藏，仅在根目录可见
