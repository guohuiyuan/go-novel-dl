...
| 站点名称 | 标识符 | 下载状态 | 说明 |
| --- | --- | --- | --- |
| ESJ Zone | `esjzone` | 可用 | 已验证登录、镜像、断点续下、简体输出 |
| WestNovel | `westnovel` | 可用 | 已验证基础下载 |
| 有度中文网 | `yodu` | 可用 | 已验证真实下载 |
| 铅笔小说 | `n23qb` | 可用 | 已验证真实下载 |
| 笔趣阁 | `biquge345` | 可用 | 已验证真实下载 |
| 飘天文学网 | `piaotia` | 可用 | 已验证真实下载，标题与作者已修正 |
| 爱下电子书 | `ixdzs8` | 可用 | 已验证 challenge 流程和章节下载 |
| 笔趣阁 | `biquge5` | 部分可用 | 已验证单章下载，长文本下载仍可能偏慢 |
| 哔哩轻小说 | `linovelib` | 部分可用 | 已接入去混淆和重试，长书下载还需优化 |
| 一笔阁 | `yibige` | 部分可用 | 已有适配器，但当前 Cloudflare 会影响稳定下载 |
| 69书吧 | `n69shuba` | 部分可用 | 已有适配器，但当前站点有 Cloudflare 挡板 |
| 笔趣阁 | `fsshu` | 部分可用 | 已校准现网页径，但完整 smoke 下载仍超时 |
