...
func postEncodedFormHTML(ctx context.Context, client *http.Client, rawURL, encodedBody string, headers map[string]string) (string, error) {
	if client == nil {
		client = http.DefaultClient
	}
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, rawURL, strings.NewReader(encodedBody))
	if err != nil {
		return "", err
	}
	req.Header.Set("User-Agent", defaultBrowserUserAgent)
