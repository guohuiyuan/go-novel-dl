...
func (s *ESJZoneSite) Download(ctx context.Context, ref model.BookRef) (*model.Book, error) {
	...
	for idx, chapter := range chapters {
		if chapter.Downloaded && chapter.Content != "" {
			chapters[idx].Order = idx + 1
			continue
		}
		content, err := s.fetchChapterContent(ctx, ref.BookID, chapter.ID)
		if err != nil {
			return nil, fmt.Errorf("fetch chapter %s/%s failed: %w", ref.BookID, chapter.ID, err)
		}
...
func (s *ESJZoneSite) fetchBookPage(ctx context.Context, bookID string) (string, string, error) {
	...
	for _, host := range s.bookAliases {
		...
		if redirected := extractMirrorRedirect(markup); redirected != "" {
			markup, err = s.get(ctx, redirected)
			if err != nil {
				lastErr = err
				continue
			}
...
func (s *ESJZoneSite) checkLoginStatus(ctx context.Context) (bool, error) {
	markup, err := s.get(ctx, s.primaryHost+"/my/favorite")
	if err != nil {
		return false, err
	}
...
func (s *ESJZoneSite) get(ctx context.Context, rawURL string) (string, error) {
